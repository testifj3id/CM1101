...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    room_identify = exits[direction]
    return rooms[room_identify]['name']
...
def print_menu(exits):
    ...
    for path in exits:
        print_menu_line(path, exit_leads_to(exits, path))
    #print_menu_line()
    # COMPLETE THIS PART:
...
def is_valid_exit(exits, user_input):
    ...
    if user_input in exits:
        exit_exists = bool(True)
    else:
        exit_exists = bool(False)
    return exit_exists
...
def menu(exits):
    ...
    while True:
        ...
        # Read player's input
        go_to = input('Select exit: ')

        # Normalise the input
        normalise_input(go_to)

        if is_valid_exit(exits, go_to) is True:
            return go_to
        else:
            print('Error')
